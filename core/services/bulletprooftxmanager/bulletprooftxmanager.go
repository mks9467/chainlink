...
func (b *BulletproofTxManager) CreateEthTransaction(newTx NewTx, qs ...postgres.QOpt) (etx EthTx, err error) {
	...
	err = q.Transaction(func(tx *sqlx.Tx) error {
		if newTx.PipelineTaskRunID != nil {
			err = tx.Get(&etx, `SELECT * FROM eth_txes WHERE pipeline_task_run_id = ? AND evm_chain_id = ?`, newTx.PipelineTaskRunID, b.chainID.String())
			if err != nil {
...
func (b *BulletproofTxManager) checkStateExists(tx *sqlx.Tx, addr common.Address) error {
	var state ethkey.State
	err := tx.Get(&state, "address = $1", addr)
...
func CheckEthTxQueueCapacity(q postgres.Queryer, fromAddress common.Address, maxQueuedTransactions uint64, chainID big.Int) (err error) {
	...
	var count uint64
	err = q.QueryRowx(`SELECT count(*) FROM eth_txes WHERE from_address = $1 AND state = 'unstarted' AND evm_chain_id = $2`, fromAddress, chainID.String()).Scan(&count)
	if err != nil {
